...
try {
    ...
    & "$RepoRoot\scripts\fetch\fetch-onnx-model.ps1" -DestDir $SmokeModels | Out-Null
    ...
    Copy-Item -Force (Join-Path $ScriptDir "onnx-config.yaml") (Join-Path $configDir "config.yaml")
