...
Search results may include **`symbol_name`**, **`symbol_kind`**, **`parent_scope`**, **`chunk_strategy`** after hybrid reindex (empty on legacy indexes). Prose files (`.md`, `.markdown`, `.mdc`, `.txt`) use `chunk_strategy: prose`, `chunk_type: markdown`, `symbol_kind` e.g. `section`/`paragraph` — no `treesitter` build required. Code AST fields need a `treesitter` binary. `snippet` is raw source text.

## When to `reindex`

- Empty search while indexing is idle
- Embedding profile changed in `config.yaml`
- Chunking settings changed (`chunking.strategy`, `chunking.version`, `languages.*`, `prose_overlap_ratio`, `context_prefix`, embed budget) or upgrade to a `treesitter` binary for AST on enabled code extensions (`.go`, `.py`, `.js`, `.jsx`, `.mjs`, `.cjs`, `.ts`, `.tsx`, `.bsl`, `.os`; `.dcs` query chunks when `languages.bsl.include_sdbl: true`)
