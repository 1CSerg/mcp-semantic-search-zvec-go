...
- Chunking settings changed (`chunking.strategy`, `chunking.version`, `languages.*`, `context_prefix`, embed budget) or upgrade to a `treesitter` binary for AST on enabled extensions (`.go`, `.py`, `.js`, `.jsx`, `.mjs`, `.cjs`, `.ts`, `.tsx`)
