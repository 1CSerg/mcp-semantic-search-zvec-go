...
- Chunking settings changed (`chunking.strategy`, `chunking.version`, `languages.*`, `context_prefix`, embed budget) or upgrade to a `treesitter` binary for AST on enabled extensions (`.go`, `.py`, `.js`, `.jsx`, `.mjs`, `.cjs`, `.ts`, `.tsx`, `.bsl`, `.os`; `.dcs` query chunks when `languages.bsl.include_sdbl: true`)
