...
|--------|-----------|------------|
| Docker (`scripts/spike/run-docker-inner.sh`) | **PASS** | `fetch-zvec-libs` + integration tests ~16 с |
| CI job `zvec-integration` | **PASS** | GitHub Actions + Docker spike |
| Windows native (MinGW CGO) | **PASS** | `scripts/dev/build-zvec-windows.ps1`; WinLibs gcc + vendor DLL |
| Phase 1 gate smoke | **PASS** | `scripts/smoke/run-phase1.ps1` — seed-index → `/v1/search` |
| Phase 2 gate smoke | **PASS** | `scripts/smoke/run-phase2.ps1` — empty project → reindex → `/v1/search` |
...
Предыдущие прогоны с `danieleugenewilliams/zvec-go` + `make deps` — см. git history этого файла. Блокеры Arrow/CMake задокументированы в [DEVELOPMENT.md](../DEVELOPMENT.md#zvec-go-phase-1) и upstream issues [#1](https://github.com/danieleugenewilliams/zvec-go/issues/1), [alibaba/zvec#468](https://github.com/alibaba/zvec/issues/468).
