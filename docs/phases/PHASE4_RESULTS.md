...
- `scripts/fetch/fetch-onnx-model.*` — download default `paraphrase-multilingual-MiniLM-L12-v2` bundle
- `scripts/fetch/fetch-onnx-runtime.*` — download ONNX Runtime shared libraries into `.deps/onnxruntime/`
- `scripts/smoke/run-phase4.*` — gate smoke without external embedding API
...
| Phase 4 smoke (Windows) | `.\scripts\smoke\run-phase4.ps1` | Pass (2026-06-08) |
