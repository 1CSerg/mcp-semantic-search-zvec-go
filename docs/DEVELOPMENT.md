...
- `.github/workflows/release.yml` — tag `v*` → binaries + Docker; platform archives upload directly to draft GitHub Release assets (not Actions artifacts); macOS release is `darwin/arm64` only (zvec-go prebuilts)
