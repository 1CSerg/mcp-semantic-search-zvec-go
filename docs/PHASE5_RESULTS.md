...
- `scripts/smoke-phase5.*` — gate smoke (3 workspaces, isolation, concurrent status, MCP proxy test)
...
| Coverage ≥88% `./internal/...`, ≥50% per package | `make test-cover-check` / `scripts/check-coverage.ps1` | Pass (88.0%) |
| Phase 5 smoke (Windows) | `.\scripts\smoke-phase5.ps1` | Pass (2026-06-08) |
