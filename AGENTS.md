...
| `zvec_open_ok: false`, LOCK error, `zvec_doc_count: 0` | Duplicate `--stdio` processes — restart Cursor; check `index_status.diagnostics` (`duplicate_stdio_suspected`); re-run install; **not** a Cyrillic-path issue — zvec stays in `INDEX_DIR/zvec/`; `reindex` with `force: true` only if zvec is empty/corrupt |
